...
- **Recipient filter:** Lambda only invokes the agent for one of a configured set of aliases on the cyndibot domain — currently `cyndi@`, `bot@`, `robot@`, `jeeves@` (configurable via `CYNDIBOT_AGENT_USERNAMES`, comma-separated). Other addresses on the domain — `pretend-mom@`, `smoketest-*@`, agent-reply round-trips — still land in S3 via the S3Action but the Lambda no-ops on them. This is what lets the agent send replies to `pretend-mom@cyndibot…` for self-loop testing without infinite recursion.
